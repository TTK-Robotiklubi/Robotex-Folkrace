...
if (reverseStartTime != 0) {
  ...
  if (millis() - reverseStartTime >= 1000) {
    ...
    turning_amplifier = 1.0;  // Safety setting while reversing

    // Calculate reverse turn bias based on sensor differences
    float reverse_turn_bias = (distances[2] - distances[0]) * 1.5; // Scale factor for sensitivity
  ...
  // If no reversing, check for new obstacles
  if (middle_distance < 15 || left_distance < 12 || right_distance < 12) {
