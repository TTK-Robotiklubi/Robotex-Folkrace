...
//const float MIN_SIGNAL_RATE = 5;  // Example threshold in MCPS //THANKS TO THIS I WONT GET PHANTOM READINGS, IF I FILTER THE READINGS WITH THIS //if 10, it wont see my legs that well, while stanging on the road. 
//minna silla ette, vaadata, mis signal rate on ja sellest natuke suurem signal rate panna
...
int getCalibratedDistance(VL53L1X &sensor, float scalingFactor, int offset) {
    ...
    signalRate = sensor.ranging_data.peak_signal_count_rate_MCPS;
    ...
    if (signalRate < signal_rate_minimal || rawDistance == 0) {
